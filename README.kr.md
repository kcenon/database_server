...
- [vcpkg](https://vcpkg.io/) 설치 및 `VCPKG_ROOT` 환경 변수 설정 (권장)

### 빠른 시작 (vcpkg)

vcpkg를 사용한 CMake 프리셋으로 빌드하는 것을 권장합니다:
...
| 프리셋 | 설명 |
|--------|------|
| `default` | vcpkg 의존성 관리를 사용한 Release 빌드 |
| `debug` | 테스트가 활성화된 Debug 빌드 |
| `release` | 최적화된 Release 빌드 |
| `dev-local` | vcpkg 없이 로컬 개발 (`FindSystemDependency` 사용) |
| `ci` | 테스트 및 벤치마크가 포함된 CI 빌드 |
| `asan` | 메모리 오류 감지용 AddressSanitizer |
| `tsan` | 데이터 레이스 감지용 ThreadSanitizer |
| `ubsan` | UndefinedBehaviorSanitizer |

> **참고**: `dev-local` 프리셋은 vcpkg를 사용하지 않으며 `FindSystemDependency` CMake 모듈을 통해 워크스페이스 상대 경로의 형제 디렉터리를 사용합니다. 기존 워크스페이스 레이아웃에서 이 프리셋을 사용하세요.
